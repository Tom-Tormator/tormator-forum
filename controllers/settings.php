...
if (isset($url[2]) and ($url[1] == "color")) {
    $result = $db->query("UPDATE users SET color='" . $db->real_escape_string($url[2]) . "' WHERE userid='" . $_SESSION["userid"] . "'");
...
if ($_SERVER['REQUEST_METHOD'] != 'POST') {
    ...
    if (isset($_POST["newusername"])) {
        ...
        if (strlen($newusername) < 1) {
            echo "New username cannot be blank.";
        }
